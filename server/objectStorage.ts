// Object Storage service for Partyline Recorder
...
export class ObjectStorageService {
  ...
  async uploadRecording(recordingSid: string, audioData: Buffer): Promise<string> {
    const privateObjectDir = this.getPrivateObjectDir();
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const fullPath = `${privateObjectDir}/recordings/${timestamp}-${recordingSid}.mp3`;
    ...
    return `/objects/recordings/${timestamp}-${recordingSid}.mp3`;
  }
